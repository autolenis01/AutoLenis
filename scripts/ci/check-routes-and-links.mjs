...
function toRouteFromPageFile(pageFile) {
  ...
  const rel = pageFile.replaceAll("\\", "/").replace(/^app\//, "").replace(/(^|\/)page\.(t|j)sx?$/, "").replace(/\/$/, "");
  if (!rel) return "/";
  return "/" + rel;
}
...
// Also collect routes from route handlers (route.ts/route.js)
const routeHandlerFiles = files.filter((f) => /\/route\.(t|j)sx?$/.test(f.replaceAll("\\", "/")));
for (const rh of routeHandlerFiles) {
  const rel = rh.replaceAll("\\", "/").replace(/^app\//, "").replace(/(^|\/)route\.(t|j)sx?$/, "").replace(/\/$/, "");
  const route = normalizeRoute(rel ? "/" + rel : "/");
