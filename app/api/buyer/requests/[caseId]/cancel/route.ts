...
export async function POST(
  ...
  try {
    ...
    const data = await sourcingService.cancelCase(caseId, buyerProfileId, session.userId)
    ...
    logger.error("[BUYER_REQUEST_CANCEL]", { error: message, correlationId })
    return NextResponse.json(
      { error: { code: 500, message: message.includes("Invalid status transition") ? message : "Unable to cancel request. Please try again." }, correlationId },
