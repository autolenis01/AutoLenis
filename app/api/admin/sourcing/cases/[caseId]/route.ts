...
export async function GET(
  ...
  try {
    ...
    logger.error("[ADMIN_SOURCING_CASE_DETAIL]", { error: String(error), correlationId })
    return NextResponse.json(
      { error: { code: 500, message: "Unable to load case details. Please try again." }, correlationId },
      { status: 500 },
    )
  }
}
