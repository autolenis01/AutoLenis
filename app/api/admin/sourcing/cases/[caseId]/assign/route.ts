...
export async function POST(
  ...
  try {
    ...
    const result = await sourcingService.assignCase(caseId, body.adminUserId, session.userId)
    ...
    logger.error("[ADMIN_SOURCING_CASE_ASSIGN]", { error: message, correlationId })
    return NextResponse.json(
      { error: { code: 500, message: message === "Case not found" ? message : "Unable to assign case." }, correlationId },
